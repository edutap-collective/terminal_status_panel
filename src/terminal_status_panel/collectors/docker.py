...
#: Categories the aggregated `/system/df` response reports, as (response key,
#: attribute name). Added to the API well after the endpoint itself -- a daemon
#: that predates them answers in lists instead, which `_disk_from_lists` reads.
_DF_CATEGORIES = (
    ("ImageUsage", "images"),
...
def _disk_from_aggregates(raw: dict, usage: DockerDiskUsage) -> bool:
    """Fill *usage* from the aggregated response form. False if it is absent."""
    seen = False
...
def _disk_from_lists(raw: dict, usage: DockerDiskUsage) -> bool:
    """Fill *usage* from the older, itemised response form.

    Docker 26.1.5 answers `/system/df` with `LayersSize` and four lists and no
    aggregates at all (measured, 2026-08-22). The panel showed `n/a` on every
    node running it, which is most of them -- distributions ship the engine
    years behind upstream, and this package cannot assume otherwise.

    The arithmetic mirrors what `docker system df` prints, because that is the
    number a reader will compare against:

    * **Images** occupy ``LayersSize`` -- deduplicated, so summing each image's
      own size would double-count shared layers. Reclaimable is that total
      minus the unique size of every image some container still uses.
    * **Containers** occupy their writable layers; only the stopped ones can be
      reclaimed.
    * **Volumes** and **build cache** are summed from their own entries, minus
      what is still referenced or in use.
    """
    images = raw.get("Images")
    containers = raw.get("Containers")
    volumes = raw.get("Volumes")
    cache = raw.get("BuildCache")
    if not any(isinstance(x, list) for x in (images, containers, volumes, cache)):
        return False

    usage.images = int(raw.get("LayersSize") or 0)
    in_use = sum(
        int(i.get("Size") or 0) - int(i.get("SharedSize") or 0)
        for i in (images or [])
        if i.get("Containers")
    )
    images_reclaimable = max(0, usage.images - in_use)

    usage.containers = sum(int(c.get("SizeRw") or 0) for c in (containers or []))
    containers_reclaimable = sum(
        int(c.get("SizeRw") or 0) for c in (containers or []) if c.get("State") != "running"
    )

    usage.volumes = sum(int((v.get("UsageData") or {}).get("Size") or 0) for v in (volumes or []))
    ...
    # A shared cache entry's bytes belong to the entry that owns them; counting
    # them here as well would report the same blocks twice.
    usage.build_cache = sum(int(b.get("Size") or 0) for b in (cache or []) if not b.get("Shared"))
    cache_reclaimable = sum(
        int(b.get("Size") or 0) for b in (cache or []) if not b.get("Shared") and not b.get("InUse")
    )

    usage.used = usage.images + usage.containers + usage.volumes + usage.build_cache
    usage.reclaimable = (
...
def _disk_failure(exc: Exception) -> str:
    """Name the failure rather than assuming one.

    The first version called everything a timeout, and the first real incident
    was spent hunting a timeout that had never happened.
    """
    text = f"{type(exc).__name__} {exc}".lower()
    return "timeout" if "timeout" in text or "timed out" in text else "unavailable"


def _disk_usage(node: str | None, root_dir: str | None, timeout: float) -> DockerDiskUsage:
    """Docker's own disk footprint, or a record saying why there is none.
    ...
    Both response forms are read, newest first. The aggregates are the modern
    shape; the lists are what every daemon older than them answers, and this
    package runs wherever it is installed rather than only on the newest
    engine.
    """
    ...
    try:
        ...
        usage.error = _disk_failure(exc)
        return usage
    if not isinstance(raw, dict):
        usage.error = "unreadable"
        return usage
    if not _disk_from_aggregates(raw, usage) and not _disk_from_lists(raw, usage):
        usage.error = "unreadable"
        return usage
